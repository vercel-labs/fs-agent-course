...
For this project we'll use Vercel Sandbox for untrusted code execution (bash commands), AI SDK, and AI Gateway.
...
What this function does is builds the full path of the `calls` folder (which is a sibling to the `agent.ts` file). Then it builds an array of all the filenames in the calls folder and builds the path for each of the files. Finally, it reads the files as buffers and writes them to the sandbox's file system.
...
If you're interested in going the extra mile, you could make a few changes to this agent to improve it and make it more flexible. You could load files from object storage or a database. Or, you could dynamically load files in a separate tool call.

Another way to improve upon this project is to implement [`bash-tool`](https://github.com/vercel-labs/bash-tool). This package abstracts the complexity of writing and reading files from the filesystem and integrates seamlessly into the Vercel Sandbox and the AI SDK.
